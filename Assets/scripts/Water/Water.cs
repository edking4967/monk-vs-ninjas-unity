...
public class Water : MonoBehaviour {
    ...
    void OnTriggerStay2D(Collider2D Hit)
    {
		...
		for (i=0; i< xpositions.Length; i++) {
			if(Hit.transform.position.x <= xpositions[i])
			{
				Debug.Log ("contacted at xposition " + (i-1) );
				break;
			}
		...
		if (i == 0)
			...
			i = i - 1;
		*/
		//if(Hit.gameObject.name == "Platform 1")
		if (Hit.gameObject.tag == "Platform")
			Hit.attachedRigidbody.AddForce (new Vector2 (0, -500 * Hit.transform.position.y  - Hit.attachedRigidbody.velocity.y * 5f ));

		
		//Hit.attachedRigidbody.AddForce (new Vector2 (0, -Hit.transform.position.y * 1000 /*+ 9.81f*/));
